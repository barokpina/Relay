...
def verify_camera_after_relay_on():
    ...
    benar-benar muncul di daftar device Windows. Hasilnya dicatat ke log
    supaya jelas: relay OK + kamera nyala, ATAU relay OK tapi kamera tidak
    terdeteksi (kemungkinan kabel/kamera bermasalah, bukan relay)."""
    global _relay_1_on
    # Beri waktu kamera untuk boot & terdaftar di Windows (DSLR biasanya 3-6 detik)
    wait_seconds = 6
    time.sleep(wait_seconds)

    # Kalau di antara waktu tunggu relay sudah dimatikan lagi, batalkan cek
    if not _relay_1_on:
        return
    ...
    if detected:
        ...
        log_message("⚠️ Relay ON tapi KAMERA TIDAK terdeteksi! Cek kabel/kamera.")
        print("[CameraCheck] PERINGATAN: relay menyala tapi kamera tidak terdeteksi di Windows.")
...
def open_relay_1():
    ...
    if _relay_1_on:
        return   # sudah ON, tidak perlu panggil DLL lagi
    if not handle:
