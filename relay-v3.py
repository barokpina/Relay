...
CAMERA_RETRY_MAX        = 5    # maksimal percobaan restart relay
CAMERA_RETRY_WAIT_AFTER_ON  = 6    # detik tunggu setelah relay ON sebelum cek
CAMERA_RETRY_WAIT_OFF_ON    = 2    # detik relay OFF sebelum dinyalakan lagi saat retry
...
def verify_camera_after_relay_on():
    ...
    Kalau kamera TIDAK terdeteksi: otomatis restart relay (OFF lalu ON
    lagi) dan cek ulang, diulang sampai CAMERA_RETRY_MAX kali atau sampai
    kamera akhirnya terdeteksi. Ini menangani kasus relay 'tidak ngangkat'
    kamera yang kadang terjadi acak (sekali beberapa jam)."""
    global _relay_1_on

    attempt = 0
    while attempt <= CAMERA_RETRY_MAX:
        ...
        if detected:
            if attempt == 0:
                ...
                log_message(f"✅ Kamera terdeteksi setelah restart relay ke-{attempt}.")
            print("[CameraCheck] Kamera terdeteksi — relay berfungsi normal.")
            return
        ...
        if attempt > CAMERA_RETRY_MAX:
            log_message(
                f"❌ Kamera TETAP TIDAK terdeteksi setelah {CAMERA_RETRY_MAX}x restart relay. "
                f"Cek fisik kamera/kabel/USB."
            )
            print("[CameraCheck] GAGAL TOTAL: kamera tidak terdeteksi setelah semua retry.")
            return

        log_message(f"⚠️ Kamera tidak terdeteksi. Mencoba restart relay (percobaan {attempt}/{CAMERA_RETRY_MAX})...")
        print(f"[CameraCheck] Restart relay percobaan {attempt}/{CAMERA_RETRY_MAX}...")

        # Matikan relay, tunggu sebentar, nyalakan lagi (tanpa verify supaya
        # tidak memicu thread verify baru — loop ini sendiri yang verify ulang)
        close_relay_1()
        time.sleep(CAMERA_RETRY_WAIT_OFF_ON)

        # Kalau relay dimatikan manual selama proses retry, hentikan
        if not running:
            return

        open_relay_1(verify=False)

        # Kalau gagal nyalakan ulang (mis. device terlepas), hentikan loop
        if not _relay_1_on:
            log_message("❌ Gagal menyalakan ulang relay saat retry, proses dihentikan.")
            return
